...
def characterize_discovered_planet(
    P_days, depth_fraction, dur_days, 
    r_star_solar=1.0, m_star_solar=1.0, t_star_k=5778.0,
    sigma_depth=None, sigma_r_star=0.03, sigma_m_star=0.04, sigma_t_star=60.0,
    v_mag=11.0
):
    """
    Astrofiziksel Karakterizasyon, Gauss Hata Yayılımı, JWST TSM ve RV Fizibilitesi.
    Bilim insanının masasına doğrudan hata paylarıyla (error bars) konur.
    """
    G = 6.67430e-11
    M_sun = 1.98847e30
    R_sun = 6.957e8
    ...
    if sigma_depth is None:
        sigma_depth = max(1e-6, depth_fraction * 0.05)

    # 1. YÖRÜNGE VE YARI-BÜYÜK EKSEN
    ...
    R_p_m = k * (r_star_solar * R_sun)
    R_p_earth = R_p_m / R_earth
    # Hata yayılımı: (sigma_Rp / Rp)^2 = (0.5 * sigma_depth / depth)^2 + (sigma_R* / R*)^2
    ...
    # 3. PROBABİLİSTİK KÜTLE TAHMİNİ (Chen & Kipping 2017)
    if R_p_earth <= 1.23:
    ...
    # 4. ORTALAMA YOĞUNLUK (g/cm^3)
    vol_cm3 = (4.0 / 3.0) * np.pi * ((R_p_earth * 6.371e8) ** 3)
    mass_g = M_p_earth * 5.972e27
    density_cgs = mass_g / vol_cm3

    # 5. TERMAL REJİM VE DENGE SICAKLIĞI (Teq)
    T_eq = float(t_star_k * np.sqrt((r_star_solar * R_sun) / (2.0 * a_m)) * (0.7 ** 0.25))
    ...
    # Güneş tipi akı cinsinden insolation (S / S_earth)
    ...
    if s_outer <= insolation_earth <= s_inner:
        hz_status = "OPTIMAL YASANABILIR BOLGE (Sıvı Su Mumkun)"
    elif insolation_earth > s_inner:
        hz_status = "SICAK BOLGE (Sera Etkisi)"
    else:
        hz_status = "SOGUK BOLGE (Buzul Rejimi)"

    # 7. JWST ATMOSFERİK GÖZLENEBİLİRLİK (TSM - Kempton et al. 2018)
    ...
    # 8. YER TABANLI RADYAL HIZ (RV) YARI-GENLİĞİ (HARPS/ESPRESSO Takip İhtiyacı)
    # K ~ (28.4 m/s) * (Mp*sin(i)/M_Jup) * (P/1yr)^(-1/3) * (M*/M_sun)^(-2/3)
