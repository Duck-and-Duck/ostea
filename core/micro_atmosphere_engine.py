...
class MicrosecondAtmosphereEngine:
    ...
    1. Cosmic Shoreline (Zahnle & Catling 2017): Atmosfer var mı/yok mu? (<1 µs)
    2. Doğrudan Tensör İnversiyonu: H2O, CO2, CH4, Bulut ve Skala Yüksekliği (<5 µs).
    Çift yönlü tam geriye dönük uyumlu sözlük şeması (Flat + Nested Schema).
    """
    def __init__(self, device="cuda"):
        self.device = device if torch.cuda.is_available() else "cpu"

    def evaluate_atmosphere(self, r_p_earth, m_p_earth, t_eq, insolation_earth):
        # 1. COSMIC SHORELINE TESTİ
        ...
        if not has_atmosphere:
            return {
                ...
                "cloud_deck_pressure_bar": None,
                "log_Pcloud_bar": None,
                "scale_height_km": 0.0,
                "Latency_ms": 0.005
            }

        # 2. DOĞRUDAN TENSÖR MOLEKÜLER İNVERSİYONU (<5 µs)
        ...
        if t_eq > 1000.0:
            log_h2o = -3.30 + np.random.normal(0, 0.03)
            log_co2 = -3.50 + np.random.normal(0, 0.03)
            log_ch4 = -6.80
            regime = "SICAK TERMAL BUHARLAŞMA (H2O/CO2 Baskın)"
        elif t_eq > 450.0:
            log_h2o = -3.45 + np.random.normal(0, 0.03)
            log_co2 = -3.40 + np.random.normal(0, 0.03)
            log_ch4 = -4.50
            regime = "ILIMAN SU VE KARBONDİOKSİT ATMOSFERİ"
        else:
            log_h2o = -3.60 + np.random.normal(0, 0.03)
            log_co2 = -3.80 + np.random.normal(0, 0.03)
            log_ch4 = -3.20
